...
- `d2d2.engine` determines which _engine_ implementation should be used for graphics and input processing. Currently, in the standard D2D2 distribution, there are two _engine_ implementations: `LwjglEngine`, which uses the LWJGL library for rendering on the client side, and `NoRenderEngine`, which does not render anything but can be useful for implementing the server-side of D2D2 applications.
